...
sys.path.append("/home1/zhc/wsi-procesing-framework/Controller/nuclick/guidingSignals.py")
...
from guidingSignals import generateGuidingSignal, jitterClicks
...
def albumentation_transform(x):
    ...
    aug = Compose([
        OneOf([
            HueSaturationValue(hue_shift_limit=0, sat_shift_limit=(-20, 20), val_shift_limit=0, always_apply=False,
                               p=.5),
            # .8
    ...
    def random_transform(self, x, weightMap, mask, seed=None):
        ...
        if transform_matrix is not None:
            ...
            weightMap = apply_transform(weightMap, transform_matrix, img_channel_axis,
                                        fill_mode="constant",
                                        cval=0.)  ### MOSSSIIII ### THIS VALUE MUST HANDEDLED BASED ON THE WEIGHT GENERATION FILE IN MATLAB
...
class NumpyArrayIterator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        batch_x = np.zeros((len(index_array),) + self.image_shape,
                           dtype=K.floatx())  # np.zeros(tuple([len(index_array)] + list(self.x.shape)[1:]), dtype=K.floatx())
        batch_weightMap = np.zeros((len(index_array),) + self.dist_shape,
                                   dtype=K.floatx())  # np.zeros(tuple([len(index_array)] + list(self.x.shape)[1:]), dtype=K.floatx())
        batch_mask = np.zeros((len(index_array),) + self.mask_shape,
                              dtype=K.floatx())  # np.zeros(tuple([len(index_array)] + list(self.x.shape)[1:]), dtype=K.floatx())
        for i, j in enumerate(index_array):
            ...
            x, weightMap, mask = self.image_data_generator.random_transform(x.astype(K.floatx()),
                                                                            weightMap.astype(K.floatx()),
                                                                            mask.astype(K.floatx()))
